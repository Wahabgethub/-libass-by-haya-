...
type PublishedStudioSuit = { id: number; productHandle: string; title: string; description: string; regularPrice: string; salePrice: string | null; publishedAt: string; updatedAt: string };
type SuitFilterMeta = { id: number; productHandle: string; color: string; style: string; season: string; updatedAt: string };
...
export async function upsertSuitFilterMeta(input: { productHandle: string; color: string; style: string; season: string }) { return mutate(state => { state.suitFilterMeta ??= []; const productHandle = input.productHandle.trim(); const color = input.color.trim(); const style = input.style.trim(); const season = input.season.trim(); if (!productHandle || !color || !style || !season) throw new Error("Color, style, and season are all required for storefront filters."); const now = new Date().toISOString(); const existing = state.suitFilterMeta.find(item => item.productHandle === productHandle); const next: SuitFilterMeta = { id: existing?.id ?? nextId(state, "suitFilter"), productHandle, color, style, season, updatedAt: now }; if (existing) Object.assign(existing, next); else state.suitFilterMeta.push(next); return { ...next, updatedAt: asDate(next.updatedAt) }; }); }
