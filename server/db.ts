...
type LocalState = { version: 1; sequences: Record<string, number>; users: LocalUser[]; categories: StoreCategory[]; categoryImages: CategoryImage[]; saleOverrides: SaleOverride[]; hiddenProducts: HiddenProduct[]; productMedia: ProductMedia[]; orders: LocalOrder[]; reviews: Review[]; delivery: DeliverySettings };
...
function initialState(): LocalState {
  ...
  return { version: 1, sequences: { user: 1, category: 1, categoryImage: 1, sale: 1, hidden: 1, media: 1, order: 1, orderItem: 1, review: 1 }, users: [], categories: [], categoryImages: [], saleOverrides: [], hiddenProducts: [], productMedia: [], orders: [], reviews: [], delivery: { freeDelivery: false, deliveryFee: "0.00", updatedAt: now } };
}
...
export async function addProductMedia(input: { productHandle: string; title: string; imageUrl: string; storageKey: string; viewLabel: string; sortOrder: number; altText?: string }) { return mutate(state => { const created: ProductMedia = { id: nextId(state, "media"), productHandle: input.productHandle, title: input.title, imageUrl: input.imageUrl, storageKey: input.storageKey, viewLabel: input.viewLabel, sortOrder: input.sortOrder, altText: input.altText ?? null, createdAt: new Date().toISOString() }; state.productMedia.push(created); return { ...created, createdAt: asDate(created.createdAt) }; }); }
