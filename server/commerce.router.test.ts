...
beforeEach(() => {
  fetchMock.mockReset();
  dbMocks.getSaleOverridesByHandles.mockReset();
...
describe("commerce.products", () => {
  ...
  it("filters a storefront-hidden product from public list and direct product access", async () => {
    dbMocks.listHiddenProductHandles.mockResolvedValue(new Set(["aria"]));
    ok({ products: { edges: [{ node: rawProduct }] } });
    ok({ productByHandle: rawProduct });

    const caller = appRouter.createCaller(makeCtx());
    await expect(caller.commerce.products.list()).resolves.toEqual([]);
    await expect(caller.commerce.products.byHandle({ handle: "aria" })).resolves.toBeNull();
  });
});
