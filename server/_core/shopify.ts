...
async function publishedStudioProducts(): Promise<Product[]> { const allSuits = await listPublishedStudioSuits(); return Promise.all(allSuits.map(async suit => { const media = await listProductMedia(suit.productHandle); const validSalePrice = suit.salePrice && Number(suit.salePrice) > 0 && Number(suit.salePrice) < Number(suit.regularPrice) ? suit.salePrice : null; const price = validSalePrice ?? suit.regularPrice; const sizes = suit.sizes ?? []; const hasSizes = sizes.length > 0; const hasSubSizes = sizes.some(s => (s.subSizes ?? []).length > 0); const options = hasSizes ? (hasSubSizes ? [{ name: "Size", values: sizes.map(s => s.label) }, { name: "Measurement", values: Array.from(new Set(sizes.flatMap(s => (s.subSizes ?? []).map(sub => sub.label)))) }] : [{ name: "Size", values: sizes.map(s => s.label) }]) : [{ name: "Title", values: ["Default Title"] }]; const variants = hasSizes ? sizes.flatMap(s => { const subs = s.subSizes ?? []; if (subs.length > 0) return subs.map(sub => ({ id: `studio-${suit.productHandle}-${slugPart(s.label)}-${slugPart(sub.label)}`, title: `${s.label} / ${sub.label}`, price: studioMoney(price), compareAtPrice: validSalePrice ? studioMoney(suit.regularPrice) : null, availableForSale: s.available && sub.available && (sub.stock === null || sub.stock === undefined || sub.stock > 0), quantityAvailable: sub.stock ?? null, selectedOptions: [{ name: "Size", value: s.label }, { name: "Measurement", value: sub.label }] })); return [{ id: `studio-${suit.productHandle}-${slugPart(s.label)}`, title: s.label, price: studioMoney(price), compareAtPrice: validSalePrice ? studioMoney(suit.regularPrice) : null, availableForSale: s.available && (s.stock === null || s.stock === undefined || s.stock > 0), quantityAvailable: s.stock ?? null, selectedOptions: [{ name: "Size", value: s.label }] }]; }) : [{ id: `studio-${suit.productHandle}-default`, title: "Default Title", price: studioMoney(price), compareAtPrice: validSalePrice ? studioMoney(suit.regularPrice) : null, availableForSale: suit.stock === null || suit.stock === undefined || suit.stock > 0, quantityAvailable: suit.stock ?? null, selectedOptions: [{ name: "Title", value: "Default Title" }] }]; const colorVariants = suit.colorGroupId ? await Promise.all(allSuits.filter(item => item.colorGroupId === suit.colorGroupId).map(async item => { const itemMedia = item.productHandle === suit.productHandle ? media : await listProductMedia(item.productHandle); return { handle: item.productHandle, title: item.title, colorLabel: item.colorLabel || item.title, imageUrl: itemMedia[0]?.imageUrl ?? null, active: item.productHandle === suit.productHandle }; })) : []; return { id: `studio-product-${suit.id}`, handle: suit.productHandle, title: suit.title, description: suit.description, descriptionHtml: "", productType: suit.productType || "Unstitched", vendor: "Libaas by HAYA", tags: [(suit.productType || "Unstitched").toLowerCase(), "studio"], images: media.map(item => ({ url: item.imageUrl, altText: item.altText ?? item.title, viewLabel: item.viewLabel })), priceRange: { min: studioMoney(price), max: studioMoney(price) }, options, variants, colorVariants, deliveryFee: suit.deliveryFee ?? null } satisfies Product; })); }
