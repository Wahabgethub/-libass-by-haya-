...
import { addCategoryImage, addFilterPreset, addHomeSectionItem, bulkAssignFilterValue, bulkUnassignFilterValue, addHomeSectionItemFromSuit, addProductMedia, createHomeSection, createStoreCategory, createStudioSuit, deleteCategoryImage, deleteStudioSuit, deleteHomeSection, deleteHomeSectionItem, deleteMotionMedia, deleteProductMedia, deleteProductMediaForProduct, deleteStoreOrder, getCategoryImageById, getDeliverySettings, getSiteBanner, getStoreOrderByNumber, hideStoreProduct, listAllCategoryImages, listAllReviews, listCategoryImages, listFilterPresets, listHomeSections, listMotionMedia, listProductMedia, listPublishedStudioSuits, listSaleOverrides, listStoreCategories, listStoreOrders, listStoreOrdersPage, listStudioSuits, listSuitFilterMeta, linkSuitColorVariant, publishStudioSuit, removeFilterPreset, removeSuitFilterMeta, unlinkSuitColorVariant, reorderMotionMedia, reorderProductMedia, saveImageUpload, updateDeliverySettings, updateReviewStatus, updateSiteBanner, updateStoreCategoryHeroImage, updateStoreOrderFulfillmentStatus, upsertSaleOverride, upsertSuitFilterMeta } from "../db";
...
export const adminRouter = router({
  ...
  suits: router({
    ...
    remove: publicProcedure.input(z.object({ adminToken, handle: z.string().min(1).max(180) })).mutation(async ({ input }) => { adminOnly(input.adminToken); await deleteStudioSuit(input.handle); return { handle: input.handle }; }),
    ...
    publish: publicProcedure.input(z.object({ adminToken, productHandle: z.string().min(1).max(180), title: z.string().min(2).max(160), description: z.string().min(10).max(2000), productType: z.string().min(1).max(60), sizes: z.array(z.object({ label: z.string().min(1).max(24), available: z.boolean(), stock: z.number().int().min(0).max(999999).nullable().optional(), subSizes: z.array(z.object({ label: z.string().min(1).max(24), available: z.boolean(), stock: z.number().int().min(0).max(999999).nullable().optional() })).max(20).optional() })).max(20).optional(), stock: z.number().int().min(0).max(999999).nullable().optional(), deliveryFee: z.number().min(0).max(999999).nullable().optional(), regularPrice: z.string().regex(/^\d+(?:\.\d{1,2})?$/), salePrice: z.string().regex(/^\d+(?:\.\d{1,2})?$/).optional() })).mutation(async ({ input }) => { adminOnly(input.adminToken); if ((await listProductMedia(input.productHandle)).length < 2) throw new Error("Add at least 2 images (Front + one more) before publishing this suit."); return publishStudioSuit(input); }),
    linkColor: publicProcedure.input(z.object({ adminToken, productHandle: z.string().min(1).max(180), colorLabel: z.string().min(1).max(40), otherProductHandle: z.string().min(1).max(180), otherColorLabel: z.string().min(1).max(40) })).mutation(({ input }) => { adminOnly(input.adminToken); return linkSuitColorVariant(input); }),
    unlinkColor: publicProcedure.input(z.object({ adminToken, productHandle: z.string().min(1).max(180) })).mutation(({ input }) => { adminOnly(input.adminToken); return unlinkSuitColorVariant(input.productHandle); }),
  }),
  suitFilters: router({
    list: publicProcedure.input(z.object({ adminToken })).query(({ input }) => { adminOnly(input.adminToken); return listSuitFilterMeta(); }),
    save: publicProcedure.input(z.object({ adminToken, productHandle: z.string().min(1).max(180), color: z.string().max(60).optional(), style: z.string().max(60).optional(), season: z.string().min(1).max(60), category: z.string().max(80).optional(), hideFromAll: z.boolean().optional() })).mutation(({ input }) => { adminOnly(input.adminToken); return upsertSuitFilterMeta(input); }),
    assignBulk: publicProcedure.input(z.object({ adminToken, kind: z.enum(["style", "size"]), value: z.string().min(1).max(60), productHandles: z.array(z.string().min(1).max(180)).min(1).max(500) })).mutation(({ input }) => { adminOnly(input.adminToken); return bulkAssignFilterValue(input); }),
    unassignBulk: publicProcedure.input(z.object({ adminToken, kind: z.enum(["style", "size"]), productHandles: z.array(z.string().min(1).max(180)).min(1).max(500) })).mutation(({ input }) => { adminOnly(input.adminToken); return bulkUnassignFilterValue(input); }),
  ...
  filterPresets: router({
    ...
    add: publicProcedure.input(z.object({ adminToken, kind: z.enum(["color", "style", "season", "size"]), value: z.string().min(1).max(60) })).mutation(({ input }) => { adminOnly(input.adminToken); return addFilterPreset(input); }),
  ...
  media: router({
    ...
    upload: publicProcedure.input(z.object({ adminToken, productHandle: z.string().min(1).max(180), title: z.string().min(1).max(160), viewLabel: z.enum(["front", "back", "detail", "editorial", "other"]), sortOrder: z.number().int().min(0).max(5), fileName: z.string().min(1).max(255), mimeType: z.enum(["image/jpeg", "image/png", "image/webp"]), dataUrl: z.string().startsWith("data:image/"), altText: z.string().max(255).optional() })).mutation(async ({ input }) => { adminOnly(input.adminToken); const uploaded = await saveImageUpload({ ...input, folder: "libaas/suits" }); return addProductMedia({ ...input, imageUrl: uploaded.url, storageKey: uploaded.storageKey }); }),
  ...
  homeSections: router({
    list: publicProcedure.input(z.object({ adminToken })).query(({ input }) => { adminOnly(input.adminToken); return listHomeSections(); }),
    create: publicProcedure.input(z.object({ adminToken, title: z.string().min(1).max(80) })).mutation(({ input }) => { adminOnly(input.adminToken); return createHomeSection(input); }),
    remove: publicProcedure.input(z.object({ adminToken, id: z.number().int().positive() })).mutation(({ input }) => { adminOnly(input.adminToken); return deleteHomeSection(input.id); }),
    addItem: publicProcedure.input(z.object({ adminToken, sectionId: z.number().int().positive(), title: z.string().max(160).optional(), linkProductHandle: z.string().max(180).optional(), fileName: z.string().min(1).max(255), mimeType: z.enum(["image/jpeg", "image/png", "image/webp"]), dataUrl: z.string().startsWith("data:image/"), altText: z.string().max(255).optional() })).mutation(async ({ input }) => { adminOnly(input.adminToken); const uploaded = await saveImageUpload({ ...input, folder: "libaas/home-sections" }); return addHomeSectionItem({ sectionId: input.sectionId, title: input.title ?? "", imageUrl: uploaded.url, storageKey: uploaded.storageKey, altText: input.altText, linkProductHandle: input.linkProductHandle }); }),
    removeItem: publicProcedure.input(z.object({ adminToken, id: z.number().int().positive() })).mutation(({ input }) => { adminOnly(input.adminToken); return deleteHomeSectionItem(input.id); }),
    addFromSuit: publicProcedure.input(z.object({ adminToken, sectionId: z.number().int().positive(), productHandle: z.string().min(1).max(180) })).mutation(({ input }) => { adminOnly(input.adminToken); return addHomeSectionItemFromSuit(input); }),
  }),
  cloudinary: router({
  ...
  orders: router({
    ...
    remove: publicProcedure.input(z.object({ adminToken, orderNumber: z.string().min(4).max(32) })).mutation(async ({ input }) => { adminOnly(input.adminToken); await deleteStoreOrder(input.orderNumber); return { orderNumber: input.orderNumber }; }),
  }),
});
