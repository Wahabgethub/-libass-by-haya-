...
export const adminRouter = router({
  ...
  suits: router({
    ...
    publish: publicProcedure.input(z.object({ adminToken, productHandle: z.string().min(1).max(180), title: z.string().min(2).max(160), description: z.string().min(10).max(2000), productType: z.string().min(1).max(60), sizes: z.array(z.object({ label: z.string().min(1).max(24), available: z.boolean() })).max(20).optional(), regularPrice: z.string().regex(/^\d+(?:\.\d{1,2})?$/), salePrice: z.string().regex(/^\d+(?:\.\d{1,2})?$/).optional() })).mutation(async ({ input }) => { adminOnly(input.adminToken); if ((await listProductMedia(input.productHandle)).length < 3) throw new Error("Add Front, Back, and Detail images before publishing this suit."); return publishStudioSuit(input); }),
  }),
  suitFilters: router({
