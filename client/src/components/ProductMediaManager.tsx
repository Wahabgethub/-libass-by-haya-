...
export function ProductMediaManager({ adminToken }: { adminToken: string }) {
  ...
  const removeSuit = trpc.admin.suits.remove.useMutation({ onSuccess: async () => { await Promise.all([utils.admin.suits.list.invalidate(), utils.commerce.products.invalidate()]); if (handle) { setHandle(""); setFiles([]); } toast.success("Suit deleted."); }, onError: error => toast.error(error.message) });
