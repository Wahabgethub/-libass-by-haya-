...
export function ProductMediaManager({ adminToken }: { adminToken: string }) {
  ...
  const createSuit = trpc.admin.suits.create.useMutation({ onSuccess: async created => { await utils.admin.suits.list.invalidate(); setHandle(created.handle); setNewSuitTitle(""); setNewSuitHandle(""); setFile(null); toast.success("Suit created. Choose its Front image first."); }, onError: error => toast.error(error.message) });
  ...
  async function uploadNext() {
    if (!suit || !file || !nextLabel) { toast.error("Choose a suit and one image first."); return; }
    if (!["image/jpeg", "image/png", "image/webp"].includes(file.type)) { toast.error("Use JPG, PNG, or WebP images only."); return; }
    if (file.size > 8 * 1024 * 1024) { toast.error("Each image must be smaller than 8 MB."); return; }
    setUploading(true);
    try {
      const form = new FormData();
      form.append("adminToken", adminToken); form.append("productHandle", suit.handle); form.append("title", suit.title); form.append("altText", `${suit.title} ${nextLabel} view`); form.append("sortOrder", String(media.length)); form.append("viewLabels", JSON.stringify([nextLabel])); form.append("files", file);
